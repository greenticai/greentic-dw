# Global Codex rules for `greentic-dw`

## Principles
- Build a Greentic-native Digital Workers kernel, not a generic agent framework.
- Keep the repo deterministic-first and extension-friendly.
- Reuse Greentic conventions for i18n, AnswerDocument, hook/sub integration, and tenant + optional team scope.
- Prefer small, surgical PRs with strong tests.

## Delivery rules
- Every PR must have a clear outcome statement.
- Preserve compatibility with the DW spec v0.2.
- Treat locale, tenant, and optional team scope as first-class.
- Keep CLI UX aligned with:
  `gtc wizard` -> `greentic-dev wizard` -> `greentic-dw wizard`
  and support `--answers`, `--schema`, `--emit-answers`, `--dry-run`.

## Quality bar
- fmt
- clippy
- tests
- docs for public contracts
